 ...
 * ==========================================================
 *
 * Library of static html components. This file must not be executed directly.
 *
 */
...
<div class="sb-main sb-chat sb-no-conversations<?php echo $css ?>" style="display: none; transition: none;">
    ...
    if ($header_type == 'brand') { 
        echo '<div class="sb-brand"><img src="'. sb_get_setting('brand-img') . '" alt="" /></div>';
                ...
                <div class="sb-text">
                    <?php echo  sb_($header_message != '' ? $header_message : 'We are an experienced team that provide fast and accurate responses to your questions.') ?>
                </div>
                <?php 
 ...
 * ----------------------------------------------------------
 *
 * Login box used by both admin and front-end
 *
 */
...
<form class="sb sb-rich-login sb-admin-box">
    <div class="sb-info"></div>
    <div class="sb-top-bar">
        <img src="<?php echo SB_URL ?>/media/logo.svg" />
        <div class="sb-title"><?php sb_e('Sign into') ?></div>  
        <div class="sb-text"><?php sb_e('To continue to Support Board') ?></div>
    </div>
    <div class="sb-main">
...
<div class="sb-main sb-admin sb-admin-start">
    <form class="sb-intall sb-admin-box">
        ...
        <div class="sb-top-bar">
            ...
            <div class="sb-text"><?php sb_e('To continue complete the installation process. Enter your database connection details below, if you are not sure about this, contact your host.') ?></div>
        </div>
        <div class="sb-main">
            ...
            <div class="sb-text"><?php sb_e('Enter the user details of the main account you will use to login into the administration area. You can update these details later.') ?></div>
...
?>
    ...
    if ($active_user != false && sb_is_agent($active_user['user_type'])) { ?>
        ...
        if ($active_user['user_type'] == 'admin') {
            ...
            <div class="sb-admin-nav-right sb-menu-mobile">
                ...
                <div class="sb-desktop">
                    ...
                    <a href="http://board.support/docs" target="_blank" class="sb-docs">
        ...
        if ($active_user['user_type'] == 'admin') {
                ...
                <div class="sb-top-bar">
                    <div>
                        ...
                        <ul class="sb-menu-wide">
                            ...
                            <?php
            if (defined('SB_WP')) {
                echo '<li id="tab-wp">WordPress</li>';
            }
            if (defined('SB_DIALOGFLOW')) {
                echo '<li id="tab-dialogflow">Dialogflow</li>';
            }
            if (defined('SB_SLACK')) {
                echo '<li id="tab-slack">Slack</li>';
            }
                            ?>
                            <li id="tab-apps">
                                <?php sb_e('Apps') ?>
            ...
            if (defined('SB_WP')) {
                echo '<div>';
                sb_populate_app_settings('wordpress') ;
                echo '</div>';
            }
            if (defined('SB_DIALOGFLOW')) {
                echo '<div>';
                sb_populate_app_settings('dialogflow') ;
                echo '</div>';
            }
            if (defined('SB_SLACK')) {
                echo '<div>';
                sb_populate_app_settings('slack') ;
                echo '</div>';
            }
                    ?>
                    <div>
                        <div class="sb-apps">
                            <div data-app="slack">
                                <?php if (defined('SB_SLACK')) echo '<i class="sb-icon-check"></i>' ?>
                                <img src="<?php echo SB_URL ?>/media/apps/slack.svg" />
                                <h2>Slack</h2>
                                <p>
                                    <?php echo sb_('Communicate with your users right from Slack. Send and receive messages and attachments, use the emoji and much more.') ?>
                                </p>
                            </div>
                            <div data-app="dialogflow">
                                <?php if (defined('SB_DIALOGFLOW')) echo '<i class="sb-icon-check"></i>' ?>
                                <img src="<?php echo SB_URL ?>/media/apps/dialogflow.svg" />
                                <h2>Dialogflow</h2>
                                <p>
                                    <?php echo sb_('Include an artificial intelligence driven chat bot to answer your users\'s questions and perform complex tasks automatically.') ?>
                                </p>
                            </div>
                        </div>
                    </div>
